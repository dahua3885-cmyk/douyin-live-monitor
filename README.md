...
| 分段与 MP4 | 默认不限时、不主动分段；停止录制后可生成 MP4，保留原始 TS；也可指定分段时长 |
...
找录像文件时，点击账号录像开关下方的「打开位置」，会在 Windows 文件资源管理器中打开最近一次实际录像所在目录。历史直播的每条录像旁也有「打开位置」，用于定位对应文件；全局录像设置里的「打开保存目录」打开总目录。没有写入过视频的账号会给出提示。
