...
| 分段与 MP4 | 默认不限时、不主动分段；录完转成 MP4 后，成品目录只展示 MP4，原文件转存内部目录；也可指定分段时长 |
...
「结构拆解」打开即显示，无需反复点击生成。直播中每分钟检查并更新草稿；确认下播且采集处理完成后自动保存结果。已结束的旧记录首次打开时自动整理，之后复用缓存；新话术或相关媒体状态变化时再更新。页面显示更新时间、直播中草稿或等待转写状态，导出也复用已保存结果。

此步骤运行关键词规则，不调用大语言模型；Whisper 模型用于前面的语音转写。更深入的语义判断尚未接入本地大模型。

第一次直接点击账号的录像开关，也会打开完整的全局「录像设置」。选择文件夹后先返回设置窗口，点击「保存并开启录像」才启用该账号；取消会放弃尚未保存的目录与参数草稿，不会开始录像。完成一次全局配置后，其他账号沿用同一套设置。

找录像文件时，点击账号录像开关下方的「打开位置」，会在 Windows 文件资源管理器中打开最近一次实际录像所在目录，并尝试将该窗口切到前台（不会永久置顶）。历史直播的每条录像旁也有「打开位置」，用于定位对应文件；全局录像设置里的「打开保存目录」打开总目录。没有写入过视频的账号会给出提示。

### 分段与累计时长有什么区别？

分段时长控制每个视频文件有多长，累计上限控制本场一共录多久。例如分段 30 分钟、累计 180 分钟，会保存约 6 个文件，属于同一场档案。分段便于及时播放、搬运和处理长直播，并减少意外中断对尚未完成文件的影响；它不能保证断电时最后一段无损。两个时长默认均为「不限时」：不主动分段，也不按累计时长停止。连续录制会在下播、手动停止或异常停录后结束；断流重连仍可能产生新文件。默认不分段时，MP4 会在这次录制结束后生成。

「浏览文件夹」直接在页面内显示本机磁盘和子文件夹，支持返回上一级及选择当前目录，也可手动填写完整路径。

### 数据保存在哪里？

- `data/monitor.sqlite3`：账号配置、采集数据、直播档案、转写文本、飞书设置等。
- `data/transcription-audio/`：用于转写和回听的音频片段。
- 自己选择的视频保存文件夹：按账号组织；录制过程中会有临时文件，录完并确认 MP4 可用后，成品目录只保留 MP4。
- `data/recording-sources/`：校验后转存的 TS 原文件、录制元数据和分段索引，用于恢复或重试。未转换成功的原文件不会清走。
