(() => {
  ...
  $('bulk-settings-form').addEventListener('submit',async event=>{
    ...
    if(changes.record_enabled&&!L.model.state?.settings?.recordings_dir){$('bulk-settings-error').textContent='请先选择录像保存文件夹，完成后再次应用批量设置。';L.openRecordFolder();return;}
  ...
  async function showArchive(id){
    ...
    const media=data.media.map(m=>`<div class="archive-media"><span>${e(L.fullTime(m.captured_at))} · ${(m.duration/60).toFixed(1)} 分钟 · ${e(m.filename)}<small>${e(({ready:'MP4 已就绪',saved:'TS 已保存',pending:'等待生成 MP4',converting:'正在生成 MP4',error:'MP4 转换失败'})[m.state]||m.state)}${m.error?' · '+e(m.error):''}</small></span><div>${m.playable?`<button type="button" class="text-button" data-play-media="${e(m.id)}">播放</button><a class="text-button" href="/api/media/${e(m.id)}?download=1" download>MP4</a>`:''}<a class="text-button" href="/api/media/${e(m.id)}?original=1" download>原 TS</a>${['saved','error'].includes(m.state)?`<button type="button" class="text-button" data-retry-media="${e(m.id)}">生成 / 重试 MP4</button>`:''}</div></div>`).join('');
